...
BOOL pifGy86_Init(PifGy86* p_owner, PifId id, PifI2cPort* p_i2c, uint16_t max_transfer_size, PifGy86Param* p_param, PifImuSensor* p_imu_sensor)
{
    ...
    if (!pifMpu60x0_Init(&p_owner->_mpu6050, PIF_ID_AUTO, p_i2c, MPU60X0_I2C_ADDR(0), max_transfer_size, p_imu_sensor)) goto fail;
    ...
    if (!pifHmc5883_Init(&p_owner->_hmc5883, PIF_ID_AUTO, p_i2c, max_transfer_size, p_imu_sensor)) goto fail;
    ...
    if (p_param && p_param->ms5611_evt_read) {
    	if (!pifMs5611_Init(&p_owner->_ms5611, PIF_ID_AUTO, p_i2c, MS5611_I2C_ADDR(1), max_transfer_size)) goto fail;
