...
static void _evtTimerDelayFinish(PifIssuerP p_issuer)
{
	...
	case MS_BREAKING:
#ifndef PIF_NO_LOG
		pifDutyMotor_SetState(p_parent, MS_STOPPING, "DMP");
#else
	...
	if (p_parent->_state == MS_GAINED) {
...
#ifndef PIF_NO_LOG
			pifDutyMotor_SetState(p_parent, MS_CONST, "DMP");
#else
		...
		if (!p_stage->rs_ctrl_duty) {
			if (p_stage->mode & MM_PC_MASK) {
...
#ifndef PIF_NO_LOG
				pifDutyMotor_SetState(p_parent, MS_LOW_CONST, "DMP");
#else
				...
				tmp_duty = 0;
#ifndef PIF_NO_LOG
				pifDutyMotor_SetState(p_parent, MS_BREAK, "DMP");
#else
...
#ifndef PIF_NO_LOG
				pifDutyMotor_SetState(p_parent, MS_REDUCE, "DMP");
#else
...
#ifndef PIF_NO_LOG
			pifDutyMotor_SetState(p_parent, MS_BREAKING, "DMP");
#else
...
#ifndef PIF_NO_LOG
		pifDutyMotor_SetState(p_parent, MS_STOP, "DMP");
#else
...
#ifndef PIF_NO_LOG
		pifDutyMotor_SetState(p_motor, MS_BREAK, "DMP");
#else
...
BOOL pifDutyMotorPos_Start(PifDutyMotorPos* p_owner, uint8_t stage_index, uint32_t operating_time)
{
    ...
    if (p_stage->p_stop_sensor) {
    	pifSensor_AttachEvtChange(p_stage->p_stop_sensor, _evtSwitchStopChange, p_owner);
    }
...
#ifndef PIF_NO_LOG
		pifDutyMotor_SetState(p_parent, MS_GAINED, "DMP");
#else
