...
BOOL pifStepMotorPos_Init(PifStepMotorPos* p_owner, PifId id, PifTimerManager* p_timer_manager, uint16_t resolution,
...
BOOL pifStepMotorPos_Start(PifStepMotorPos* p_owner, uint8_t stage_index, uint32_t operating_time)
{
    ...
    if (pstStage->p_stop_sensor) {
    	pifSensor_AttachEvtChange(pstStage->p_stop_sensor, _evtSwitchStopChange, p_owner);
    }
