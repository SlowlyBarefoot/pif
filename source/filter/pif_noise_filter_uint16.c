...
static void _clear(PifNoiseFilterUint16* p_owner)
{
...
{
	PifNoiseFilterUint16* p_owner = (PifNoiseFilterUint16*)p_parent;
...
BOOL pifNoiseFilterUint16_Init(PifNoiseFilterUint16* p_owner, uint8_t size)
{
    ...
    memset(p_owner, 0, sizeof(PifNoiseFilterUint16));

	p_owner->__buffer = calloc(size, sizeof(uint16_t));
	if (!p_owner->__buffer) {
