...
static void _evtTimerDelayFinish(void *pvIssuer)
{
	...
	default:
		...
		pifLog_Printf(LT_enWarn, "DMP(%u) S:%u", pstOwner->_usPifId, pstOwner->_enState);
#endif
		break;
	}
}
...
static void _fnControlPos(PIF_stDutyMotor *pstOwner)
{
	...
	if (enState != pstOwner->_enState && pif_stLogFlag.bt.DutyMotor) {
		pifLog_Printf(LT_enInfo, "DMP(%u) %s D:%u(%u%%) CP:%u", pstOwner->_usPifId,
 ...
 * @return 
 */
PIF_stDutyMotor *pifDutyMotorPos_Create(PIF_usId usPifId, PIF_stPulse* p_timer, uint16_t usMaxDuty, uint16_t usControlPeriod)
{
